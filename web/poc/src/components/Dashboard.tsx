...
export function Dashboard() {
  const [apiConfig] = useState<ApiConfig>({ baseUrl: getBaseUrl() })
  ...
  // Uses an authenticated probe (GET /v1/channels?limit=1) to verify the proxy
  // key injection is working. A 401 means BACKOFFICE_API_KEY is not set on the server.
  const runCheckConnection = useCallback(async (config: ApiConfig) => {
    ...
    try {
      ...
      if (err instanceof ApiError && err.status === 401) {
        toast.error('API key not configured on the server', {
          description: 'Set BACKOFFICE_API_KEY in the frontend container environment and restart.',
        })
      }
    }
  }, [])
    ...
    try {
      ...
      const message =
        err instanceof ApiError
          ? err.status === 401
            ? 'API key not configured on the server — set BACKOFFICE_API_KEY and restart.'
  ...
  useEffect(() => {
    void runCheckConnection(apiConfig)
    void runLoadSpaces(apiConfig)
    void runLoadMerchants(apiConfig)
