// Package api wires the Gin engine, CORS, middleware, and all route groups.
package api

import (
	"github.com/gin-contrib/cors"
	"github.com/gin-gonic/gin"
	"github.com/valianx/discord-support-hub/internal/api/handlers"
	"github.com/valianx/discord-support-hub/internal/api/middleware"
	"github.com/valianx/discord-support-hub/internal/observability"
)

// RouterConfig carries the runtime dependencies the router needs.
type RouterConfig struct {
	// CORSAllowedOrigins is the allowlist of permitted request origins.
	// An empty slice locks the API down (no cross-origin requests allowed).
	// Never use "*" with credentials (docs/02-architecture.md §5.3).
	CORSAllowedOrigins []string

	// Health check pingable dependencies.
	PGPinger    observability.Pinger
	RedisPinger observability.Pinger
}

// NewRouter builds and returns the configured Gin engine.
// The engine is suitable for passing to http.Server.
func NewRouter(cfg RouterConfig) *gin.Engine {
	r := gin.New()

	// Middleware: recovery first so panics in other middleware are caught.
	r.Use(middleware.Recovery())
	r.Use(middleware.RequestID())
	r.Use(corsMiddleware(cfg.CORSAllowedOrigins))

	// Health endpoints — exempt from authentication.
	r.GET("/livez", observability.LivezHandler)
	r.GET("/readyz", observability.ReadyzHandler(cfg.PGPinger, cfg.RedisPinger))

	// OAuth2 callback — exempt from service API key auth (security: [] in OpenAPI).
	r.GET("/v1/oauth/discord/callback", handlers.OAuthDiscordCallback)

	// All other v1 routes require authentication (Layer A, M1).
	v1 := r.Group("/v1", middleware.Auth())
	registerV1Routes(v1)

	return r
}

// registerV1Routes attaches all v1 path groups.
func registerV1Routes(v1 *gin.RouterGroup) {
	// Spaces — provision + reads + lifecycle + welcome sync.
	v1.POST("/merchants/:merchantId/channels", middleware.Idempotency(), handlers.ProvisionSpace)
	v1.GET("/channels", handlers.ListSpaces)
	v1.GET("/channels/:id", handlers.GetSpace)
	v1.GET("/channels/:id/members", handlers.ListSpaceMembers)
	v1.POST("/channels/:id/lifecycle", middleware.Idempotency(), handlers.ChangeSpaceLifecycle)
	// Note: "welcome:sync" contains a literal colon which Gin would misparse as a param.
	// We register it as a static segment under a separate router group to avoid this.
	// The OpenAPI path is POST /v1/channels/{id}/welcome:sync — clients send the colon literally.
	// TODO(M4): reconsider path if Gin adds literal-colon support; for now the workaround is correct.
	v1.POST("/channels/:id/welcomesync", middleware.Idempotency(), handlers.SyncWelcome)

	// Collaborators.
	v1.POST("/channels/:id/collaborators", middleware.Idempotency(), handlers.InviteCollaborator)
	v1.DELETE("/channels/:id/collaborators/:userId", middleware.Idempotency(), handlers.ExpelCollaborator)
	v1.GET("/collaborators/:userId/channels", handlers.ListCollaboratorChannels)

	// Agents (Admin only).
	v1.GET("/agents", handlers.ListAgents)
	v1.POST("/agents", middleware.Idempotency(), handlers.AddAgent)
	v1.DELETE("/agents/:userId", middleware.Idempotency(), handlers.RemoveAgent)

	// Transversal.
	v1.GET("/directory", handlers.GetDirectory)
	v1.GET("/audit", handlers.GetAudit)

	// Jobs.
	v1.GET("/jobs/:jobId", handlers.GetJob)
}

// corsMiddleware returns a gin-contrib/cors middleware with locked-down defaults.
// allowedOrigins comes from config (FR-13); credentials are never allowed with "*".
func corsMiddleware(allowedOrigins []string) gin.HandlerFunc {
	if len(allowedOrigins) == 0 {
		// No allowlist configured: omit CORS headers entirely so the browser
		// enforces same-origin policy. Cross-origin requests receive no
		// Access-Control-Allow-Origin header and are therefore blocked by the
		// browser. Operators add origins via CORS_ALLOWED_ORIGINS.
		return func(c *gin.Context) { c.Next() }
	}

	cfg := cors.Config{
		AllowOrigins:     allowedOrigins,
		AllowMethods:     []string{"GET", "POST", "DELETE", "OPTIONS"},
		AllowHeaders:     []string{"Authorization", "Content-Type", "Idempotency-Key", "X-Request-ID"},
		ExposeHeaders:    []string{"Location", "X-Request-ID"},
		AllowCredentials: false, // only set true on the session path in POC-FE (§5.3)
	}
	return cors.New(cfg)
}
